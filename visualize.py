...
PALETTE = ["#EBD4FF", "#B8D5DD", "#DDF4D7", "#F4ECD7", "#FFD7D4"]
...
.metric-value { font-size: 1.6rem; font-weight: 700; color: #4a6b78; }
...
.stButton > button:hover { background-color: var(--c-pink); }

/* 청크 탐색 expander 박스 */
[data-testid="stExpander"] { background-color: var(--c-green); border-radius: 8px; }
</style>
...
with tab_overview:
    ...
    with left:
        ...
        fig = px.bar(sc, x="청크 수", y="source", orientation="h",
                     color="청크 수", color_continuous_scale=["#F4ECD7", "#B8D5DD"])
    ...
    with right:
        ...
        fig = px.histogram(df, x="length", nbins=40, color_discrete_sequence=["#B8D5DD"],
                           labels={"length": "글자 수"})
